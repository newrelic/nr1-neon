...
export default class MotherBoard extends React.Component {
  ...
  addBoard() {
    ...
    if (boardName && eventName && boardName.trim !== '' && eventName.trim() !== '') {
  ...
  generateUUID(a,b) {for(b=a='';a++<36;b+=a*51&52?(a^15?8^Math.random()*(a^20?16:4):4).toString(16):'-');return b}
      ...
      <div>
        <section className="board_list">
          ...
          <article className="board_card">
            <a href="#" onClick={(e) => this.showAddBoardModal(e)}>
              <Icon type={Icon.TYPE.INTERFACE__SIGN__PLUS} sizeType={Icon.SIZE_TYPE.LARGE} style={{transform: 'scale(6)'}} />
            </a>
          </article>
        </section>
        <Modal hidden={modalHidden} onClose={this.closeModal}>
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr', gridGap: '1em'}}>
            <TextField label="Board Name" placeholder="" onChange={(e) => this.textUpdated(e, 'board')} value={boardName} />
            <TextField label="Event Name" placeholder="" onChange={(e) => this.textUpdated(e, 'event')} value={eventName} />
            <Button iconType={Button.ICON_TYPE.INTERFACE__SIGN__PLUS} onClick={this.addBoard}>Add</Button>
          </div>
        </Modal>
      </div>
    )
