...
export default class Select extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (!(prevPropVal.length === propVal.length && prevPropVal.every((v, i) => v === propVal[i]))) {
      this.setState({
        values: propVal
      })
    }

    if (!(prevStateVal.length === stateVal.length && prevStateVal.every((v, i) => v === stateVal[i]))) this.props.onChange(stateVal)
